...
export const Verification = ({ match }) => {
    ...
    const submitHandler = () => {
        ...
        if (form.data.type === 'registration') {
            ...
            localStorage.setItem('tms_app_db', JSON.stringify([...__DB.data, ainfo]))
            __DB.set([ainfo])
        }

        localStorage.setItem('tms_app_session', JSON.stringify(ainfo))
        ...
        form.data.type !== 'malformed' ? (
            <Box width="100%" height="100%">
                <Box display="flex" width="100%" height="100%" borderRadius={4}>
                    <Box width={smdevice ? '100%' : '100%'} display="flex" alignItems="center" justifyContent="center" bgcolor="#fff" p={5}>
                        <Box width={320}>
                            <Box fontSize={20} fontWeight={800} color="#009E6A">{form.data.type === 'login' ? 'Welcome back' : 'Register Account'}</Box>
                            ...
                            <Box display="flex" flexDirection="column" gap={3} mt={1.5}>
                                ...
                                />
                                <Box component={Button} onClick={submitHandler} borderRadius={2} variant="contained"
                            ...
                            <Box display="flex" justifyContent="center" gap={1} mt={5} fontSize={13}>
                                <Box>{form.data.type === 'login' ? `Don't` : 'Already'} have an account? </Box> <Box color="#009E6A" sx={{cursor: 'pointer'}} onClick={redirectHandler}>{form.data.type === 'login' ?'Register' : 'Login'}</Box>
                            </Box>
                        </Box>
                    </Box>
                </Box>
