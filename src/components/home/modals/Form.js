...
import { CstmTextField } from "../../../core/global/CstmTextField";
                ...
                <Box width={sxdevice ? '100%' : 600} height="100%" display="flex" flexDirection="column" bgcolor="#fff" borderRadius={sxdevice ? 0 : 3}>
                    <Box px={3} pt={3}>
                        <Box display="flex" alignItems="center" justifyContent="space-between">
                            <Box fontSize={14} fontWeight={600}>{form.data.method === 'create' ? 'Create' : 'Edit'} Task</Box>
                            <Box component={Button} minWidth="" onClick={() => formHandler(null, false)}><Close sx={{fontSize: 20, color: '#333'}} /></Box>
                        </Box>
                    </Box>
                    <Box display="flex" flexDirection="column" gap={2} sx={{overflow: 'auto'}} px={3} pb={3} pt={4}>
                        <Box>
                            <Box fontSize={14} fontWeight={600} mb={0.5}>Date</Box>
                        ...
                        <Box display="flex" flexDirection="column" gap="4px">
                            <Box fontSize={14} fontWeight={600}>Title</Box>
                            <CstmTextField variant="outlined" name="title" fullWidth sx={tfsx()} onInput={inputHandler}
                                value={inputs.title.value} error={inputs.title.stat} helperText={inputs.title.msg}
                            />
                        </Box>
                        <Box display="flex" flexDirection="column" gap="4px">
                            <Box fontSize={14} fontWeight={600}>Task</Box>
                            <CstmTextField multiline rows={4} name="task" variant="outlined" fullWidth sx={tfsx('area')}
                        ...
                        <Box display="flex" flexDirection="column" alignItems="flex-start" gap={2}>
                            <Box display="flex" alignItems="center" justifyContent="flex-end" width="100%">
                                <Box bgcolor="primary.main" color="#FFF" borderRadius={10} display="flex" alignItems="center" fontSize={12} pr={2}>
                        ...
                        <Box>
                            ...
                            <Box {...getRootProps()} minHeight={116} textAlign="center" bgcolor="#F1F3F4" border="2px dashed #D7DDDF" borderRadius={2} p={2} sx={{cursor: 'pointer'}}>
                                <Box component="label" htmlFor="attach_file">
                                    <input {...getInputProps()} />
                                    <CloudUploadFill1 fill="#009E6A" width={30} height={24} />
                                    <Box fontSize={14} fontWeight={400} color="#009E6A">Click to upload or drag & drop</Box>
                                </Box>
                            </Box>
                            <Box mt={1.5}>
                    ...
                    <Box mt="auto" px={3} pb={3}>
                        <Box component={Button} onClick={() => confirmHandler(true, form.data.method === 'create' ? 'add' : 'update')} fullWidth bgcolor={disableBTNHandler() ? "#F1F3F4":"#009E6A"} color="#fff" sx={{':hover': {bgcolor: '#009E6A'}}}
